...
def authorize(envelope):
    ...
    for grant in policy.get("direct_grants", []):
        if (
            ...
            and envelope["origin_surface"] in grant.get("origin_surfaces", [])
        ):
            return
    fail("actor has no independent institutional grant for review-disposition")
